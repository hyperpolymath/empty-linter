...
image:https://img.shields.io/badge/Idris_Inside-proven-purple.svg[Idris
Inside]
...
https://github.com/hyperpolymath/proven[proven library] with Idris 2
dependent types.
...
|SafeWhitespace |Text normalization without data loss (NEW)
|`+TextTransform.res+`
|===

Operations marked with "`Idris Inside`" have compile-time proofs that
they cannot crash or corrupt data.

== Crap-Overlay Playbook

[arabic]
. *Audit:* Identify invisible artifacts.
. *Highlight:* Magenta-coded reporting of offsets.
. *Correct:* 0xA0 to 0x20 conversion and ZWSP stripping.

== Quick Start (Bash)

The primary entry point for Bash is provided below. For all other
supported shells (including `+nushell+`, `+fish+`, `+minix+` `+shell+`,
`+elvish+`, etc.), please refer to link:docs/SHELLS.adoc[The Multi-Shell
Registry].
...
link:cookbook.adoc[Cookbook] for a full list of recipes.
